...
public sealed class StopLiveStreamCommandHandler(
    IConferDbContext dbContext,
    ISignalingNotifier signalingNotifier)
    : ICommandHandler<StopLiveStreamCommand, Result<LiveStreamResponse>>
{
    public async Task<Result<LiveStreamResponse>> HandleAsync(
        StopLiveStreamCommand command,
        CancellationToken cancellationToken = default)
    {
        ...
        var streamResult = meeting.StopLiveStream(command.ActorId);
