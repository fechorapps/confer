...
pub fn render_waiting_lobby(app: &mut ConferApp, ui: &mut Ui) {
    ...
    egui::Frame::group(ui.style())
        ...
        .show(ui, |ui| {
            ui.horizontal(|ui| {
                ui.label(RichText::new("CONFER").size(16.0).strong().color(Color32::from_rgb(56, 189, 248)));
                ui.label(RichText::new("•").color(Color32::from_rgb(71, 85, 105)));
                ui.label(RichText::new("Waiting Room").size(14.0).strong().color(Color32::from_rgb(248, 250, 252)));

                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if ui.add(egui::Button::new(RichText::new("✕ Leave Meeting").size(12.0).color(Color32::WHITE)).fill(Color32::from_rgb(225, 29, 72)).rounding(6.0)).clicked() {
                        app.leave_meeting();
                    }
                });
            });
    ...
    ui.vertical_centered(|ui| {
        ...
        egui::Frame::group(ui.style())
            ...
            .show(ui, |ui| {
                ...
                let (rect, _response) = ui.allocate_exact_size(Vec2::new(card_width, 80.0), egui::Sense::hover());
                ...
                // Status message
                let waiting_msg = app.waiting_room_message.as_deref().unwrap_or("Please wait, the meeting host will let you in soon.");
                ui.label(RichText::new(waiting_msg).size(13.0).color(Color32::from_rgb(148, 163, 184)));
                    ...
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ...
                            egui::Frame::group(ui.style())
                                ...
                                .inner_margin(8.0)
                                .show(ui, |ui| {
                                    ui.label(RichText::new(initial).size(14.0).strong().color(Color32::WHITE));
                                });

                            ui.add_space(6.0);

                            ui.vertical(|ui| {
                                ui.label(RichText::new(&app.user_display_name).size(13.0).strong().color(Color32::from_rgb(248, 250, 252)));
                            ...
                            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                egui::Frame::group(ui.style())
                                    ...
                                    .show(ui, |ui| {
                                        ui.label(RichText::new("WAITING").size(10.0).strong().color(Color32::from_rgb(251, 191, 36)));
                                    });
                            });
                        });
                    });
                ...
                ui.horizontal(|ui| {
                    ui.label(RichText::new("Pre-join check:").size(11.0).color(Color32::from_rgb(100, 116, 139)));

                    let mic_icon = if app.is_mic_muted { "🔇 Mic Muted" } else { "🎙 Mic Ready" };
                    let mic_col = if app.is_mic_muted { Color32::from_rgb(244, 63, 94) } else { Color32::from_rgb(16, 185, 129) };
                    ui.label(RichText::new(mic_icon).size(11.0).color(mic_col));

                    ui.label(RichText::new("•").color(Color32::from_rgb(71, 85, 105)));

                    let cam_icon = if app.is_camera_off { "📷 Camera Off" } else { "🎥 Camera Ready" };
                    let cam_col = if app.is_camera_off { Color32::from_rgb(244, 63, 94) } else { Color32::from_rgb(16, 185, 129) };
                ...
                if ui.add(
                    egui::Button::new(RichText::new("✕ Leave Waiting Room").size(13.0).strong().color(Color32::WHITE))
